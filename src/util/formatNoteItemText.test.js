...
test("returns 'No note text' when text is undefined", () => { 
  const formattedText = formatNoteItemText(undefined); expect(formattedText).toBe(emptyNoteTextPlaceholder);
});
...
test("returns 'No note text' when text is empty", () => { 
  const formattedText = formatNoteItemText(""); expect(formattedText).toBe(emptyNoteTextPlaceholder);
});
...
test("returns text unchanged when it's fewer than 200 characters", () => { 
  ...
  const formattedText = formatNoteItemText(text); expect(formattedText).toBe(text);
});
